...
public class CleanFragment extends Fragment {
    ...
    private void getTopList() {
        viewModel.makeApiCallCleanArticles().observe(requireActivity(), response -> {
    ...
    private void onSwipeRefresh() {
        binding.swipeToRefreshArticles.setOnRefreshListener(() -> viewModel.makeApiCallCleanArticles().observe(requireActivity(), response -> {
            switch (response.status){
                case ERROR: {
    ...
    private void initializeViewModel() {
        viewModel = new ViewModelProvider(this).get(CleanViewModel.class);
    }
}